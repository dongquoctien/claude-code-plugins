...
/feature-mockup:status "<feature-name>"   # detail of one feature
/feature-mockup:status                     # list all features and their phase
```

Each feature folder has a `STATUS.md` (auto-regenerated by every skill) plus a hidden `.timeline.json` (chronological event log). Returning a day later? `/preview` or `/verify` will show a one-line resume hint, and `STATUS.md` always reflects the latest state.
...
| `/feature-mockup:status` | BA | Show what's been done, pending issues, and suggested next step for a feature (or list all features when no argument given) |
...
- **Phase 3 (shipped):** `preview` skill (browser auto-launch), `verify` skill (Chrome DevTools / Playwright MCP comparison), `fix` skill (UI + interaction fixes with theme guardrails), `deploy` skill (Netlify Drop / Surge.sh / Cloudflare Pages / GitHub Pages publishing), `status` skill + per-feature `STATUS.md` + `.timeline.json` so the BA can resume work after a break.
